...
from app.utils.datetime_utils import utc_now, to_local, format_russian_datetime
...
async def cmd_remind(message: Message, state: FSMContext, user: User, user_tz: str):
    ...
    if not text:
        ...
        await message.answer("✍️ Напишите, о чём и когда вас напомнить:")
        return
...
async def handle_freeform_text(message: Message, state: FSMContext, user: User, user_tz: str, session: AsyncSession):
    """Handle freeform natural language text input."""
    current_state = await state.get_state()
    text = message.text.strip()

    # Check for FSM specific states
    ...
    if current_state == CreateReminderStates.waiting_for_edit_time.state:
        ...
        if parsed.target_datetime:
            ...
            await message.answer("⚠️ Не удалось распознать дату и время. Попробуйте написать, например: <i>«Завтра в 15:00»</i>", parse_mode="HTML")
        return

    # Check for context phrases like "перенеси на завтра", "перенеси на 18:00"
    ...
    # Default flow: parse text as new reminder
    await process_reminder_text(message, text, state, user, user_tz)
...
async def callback_edit_draft(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("✏️ Напишите новый текст или новое время для напоминания:", reply_markup=get_main_menu_keyboard())
    await call.answer()
...
async def callback_cancel_draft(call: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await call.message.edit_text("❌ Создание напоминания отменено.", reply_markup=get_main_menu_keyboard())
    await call.answer()
...
async def callback_clarify_time(call: CallbackQuery, callback_data: ClarifyTimeCallback, state: FSMContext, user: User, user_tz: str):
    ...
    if preset == "15m":
        ...
        await call.message.edit_text("✍️ Напишите время (например: <i>«завтра в 18:00»</i>):", parse_mode="HTML")
        await call.answer()
        return
